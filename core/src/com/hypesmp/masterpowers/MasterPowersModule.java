...
public final class MasterPowersModule implements Listener, CommandExecutor, TabCompleter {
    ...
    private boolean unlocked(Player p) {
        return p.isOp() || getPlaytime(p) >= config.getLong("unlock-playtime-seconds", 129600);
    }
    ...
    private void use(Player p, String raw) {
        ...
        switch (power) {
            ...
            case "water" -> {
                ...
                for (int i = 1; i <= 6; i++)
                    p.getWorld().spawnParticle(Particle.WATER_SPLASH, p.getLocation().add(v.clone().multiply(i)).add(0, 1, 0), 12, .3, .3, .3, .1);
            ...
            case "earth" -> {
                ...
                p.getWorld().spawnParticle(Particle.BLOCK_CRACK, l, 50, .7, .3, .7, .1, Material.DIRT.createBlockData());
            ...
            case "jump" -> {
                p.addPotionEffect(new PotionEffect(PotionEffectType.JUMP, config.getInt("duration.jump", 10) * 20, config.getInt("amplifier.jump", 3), false, false, true));
                premiumEffect(p, power);
                cd(p, power);
            }
            case "shield" -> {
                p.addPotionEffect(new PotionEffect(PotionEffectType.DAMAGE_RESISTANCE, config.getInt("duration.shield", 8) * 20, config.getInt("amplifier.shield", 2), false, false, true));
                premiumEffect(p, power);
                cd(p, power);
            }
            case "invisibility" -> {
    ...
    private void premiumEffect(Player p, String power) {
        ...
        p.spigot().sendMessage(net.md_5.bungee.api.ChatMessageType.ACTION_BAR, new net.md_5.bungee.api.chat.TextComponent(color("&d✦ &l" + serverName + " &5| &f" + capitalize(power) + " &dPOWER ACTIVATED ✦")));
        p.getWorld().spawnParticle(Particle.END_ROD, p.getLocation().add(0, 1, 0), 18, .45, .65, .45, .03);
        p.getWorld().spawnParticle(Particle.ENCHANTMENT_TABLE, p.getLocation().add(0, 1, 0), 28, .65, .8, .65, .05);
    }
    ...
    public void onDamage(EntityDamageEvent e) {
        if (e.getEntity() instanceof Player p && hasPower(p, "shield") && p.hasPotionEffect(PotionEffectType.DAMAGE_RESISTANCE)) {
